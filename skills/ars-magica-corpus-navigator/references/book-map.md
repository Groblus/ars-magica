# Book Map

Generated resources are authoritative for exact line spans. This file is a human-facing shelf map.

## P0 Core

- `resources/corpus/Ars Magica - Definitive Edition (Core Rules).md`: rules authority for Definitive Edition.

## P1 Early Play

- `resources/corpus/Ars Magica 5e - Covenants.md`: covenant toolkit.
- `resources/corpus/Ars Magica 5e - Houses of Hermes - True Lineages.md`: Bonisagus, Guernicus, Mercere, Tremere.
- `resources/corpus/Ars Magica 5e - Houses of Hermes - Societates.md`: Flambeau, Jerbiton, Tytalus, Ex Miscellanea.

## Houses

- `Houses of Hermes - Mystery Cults`: Bjornaer, Criamon, Merinita, Verditius.

## Realms

- `Realms of Power - Magic`
- `Realms of Power - Faerie`
- `Realms of Power - The Divine (Revised)`
- `Realms of Power - The Infernal`

## Mundane Society

- `Art & Academe`
- `City & Guild`
- `Lords of Men`
- `The Church`

## Regions

- `Guardians of the Forests - The Rhine Tribunal`
- `The Lion and the Lily - The Normandy Tribunal`

## Toolkits

- `Magic - Apprentices`
- `Magic - Hedge Magic (Revised)`
- `The Mysteries (Revised)`
- `Mythic Locations`
- `Transforming Mythic Europe`
